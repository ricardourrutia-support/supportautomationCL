    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for aud in audiences:
            ...
            for row_num in range(len(df_aud)):
                fmt = alt_row_format if row_num % 2 == 0 else normal_format
                for col_num in range(len(df_aud.columns)):
                    cell_value = df_aud.iloc[row_num, col_num]
    ...
    for aud in ['Driver', 'Rider', 'B2B', 'Emergencias', 'Aeropuerto', 'Aeropuerto WhatsApp']:
        ...
        if len(df_trend) > 1:
            ...
            try:
                ...
                fig_firt = crear_grafico_evolucion(df_trend, 'FiRT <24h (%)', '#FF6B6B', 'FiRT <24h (%)', y_range=[0, 100])
...
if file_main is not None:
    ...
    with tabs[1]:
        ...
        if len(df_trend) > 0:
            ...
            with col1:
                fig_firt = px.area(df_trend, x='PeriodLabel', y='FiRT <24h (%)', 
                                  color_discrete_sequence=[CABIFY_PURPLE])
            ...
            with col2:
                fig_furt = px.area(df_trend, x='PeriodLabel', y='FuRT <36h (%)',
                                  color_discrete_sequence=[CABIFY_SECONDARY])
            ...
            df_table.columns = ['Período', 'Volumen', 'NPS', 'CSAT (%)', 'FiRT <24h (%)', 'FuRT <36h (%)', 'TMO (Hrs)', 'Reopen (%)']
    ...
    # === TAB 3: DEEP DIVE ===
    with tabs[2]:
        st.markdown(f"### 🔍 Deep Dive: ¿Qué dicen nuestros detractores? ({selected_audience} - {period_display})")
    ...
    if df_brandwatch is not None and len(tabs) > 3:
        with tabs[3]:
            ...
            fig_cat = px.bar(cat_counts, x='Volumen', y='Categoría', orientation='h',
                            color='Categoría', color_discrete_map=color_map)
            fig_cat.update_layout(yaxis={'categoryorder':'total ascending'}, plot_bgcolor="white", showlegend=False)
